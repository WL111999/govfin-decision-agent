...
**先做这一步，否则你在界面里看不到任何集成效果。**

Nexent 是多租户架构，智能体开发界面全部活在租户内部。部署时自动创建的 `suadmin`
是**平台管理员**（`user_role=SU`），它的可访问路由只有 `/`、`/resource-manage`、`/space`
三项——侧边栏就只有「首页」和「资源管理」，看不到智能体开发入口。**这是设计，不是故障。**

这个脚本会一次做完：

1. 建一个演示租户
2. 给租户发管理员邀请码
3. 邀请码注册一个租户管理员账号（**会打印邮箱和密码**）
4. 用该账号在**新租户下**注册 govfin 的 MCP 服务
5. 导入 3 份 Skill 模板

跑完用打印出来的账号登录 `http://localhost:3000`，侧边栏会展开完整菜单。

> 注册邮箱不能用 `.local` 这类保留域名，校验层会拒掉。脚本默认用
> `govfin@nexent-demo.com`。

#### 为什么不能只注册 MCP 就完事

MCP 记录和 Skill **都是按租户隔离的**（`get_mcp_records_by_tenant(tenant_id=)`）。
用 suadmin 注册，记录会挂在一个占位租户（`DEFAULT_TENANT_ID = "tenant_id"`）下，
真正的租户用户登录后**看不到它们**。必须在正确的租户下注册。

### 4. 手动注册 MCP 服务（可选）

跑过第 3 步之后，这一节就**不用再做了**——脚本已经把 MCP 服务和 Skill 都装好了。
这里写的是界面上手动操作时的对应路径，用来核对脚本到底做了什么，或者脚本跑不通时的备用方案。

> `scripts/register_to_nexent.py` 也能注册，但它是按**当前登录身份**的租户来的。
> 用 suadmin 的 token 跑，记录会挂在占位租户下。**正确做法是先用第 3 步建好租户**，
> 再登录租户账号去注册——`provision_tenant.py` 内部就是这么做的。
...
用第 3 步打印的租户账号登录，在 Nexent 里新建智能体，工具选中全部 10 个 `govfin` 工具，
再按 [Skill 注册](#skill-注册) 挂上三个工作流模板（第 3 步已经导入过了）。
